...
import { IconName, iconNames } from "~/components/types/icon";
...
const ClickableIcon = ({ name, size, ...props }: IconProps) => {
  ...
  return (
    <Icon
      ...
      name={clicked ? "Lucide/check" : name}
      size={size}
      onClick={copyIcon}
    />
  );
  ...
  return (
    <div className="flex flex-col gap-4 p-2">
      <div className="flex items-center justify-end gap-2">
        <label className="text-white" htmlFor="classes">
          Icon classes:
        </label>
        <input
...
export const iconLibraryPlugin = () => ({
  icon: <Icon name="Lucide/shoppingCart" size="sm" />,
